...
declare function local:allTeiVolumes($my_collection as xs:string, $first_name as xs:string, $last_name as xs:string) as node()* {
        ...
        <entry type="xml" name="{teigeneration:get_filename_from_ref($vol)}">
            {
                ...
                $my_collection, $vol, $first_name, $last_name)
...
return
    response:stream-binary(
        xs:base64Binary(compression:zip(local:allTeiVolumes($my_collection, $first_name, $last_name), false()) ),
    'application/zip',
        $collectionName || ".zip")
