...
let $starting_span := request:get-parameter('starting-span','')
...
update  insert <html:span xmlns:html="http://www.w3.org/1999/xhtml" class="cts_picker" id="{$span_id}" data-ctsurn="{$value}" title="{$name || ' = ' || $value}" data-starting-span="{$starting_span}">📖<html:button class="kill_button" type='button' id="{$button_id}"> <html:span>×</html:span> </html:button></html:span> preceding $next_sibling
